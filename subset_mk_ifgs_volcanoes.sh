...
echo "same subset_mk_ifgs.sh but only create the interferograms from list"
if [ -z $1 ]; then
 echo "Usage e.g.: subset_mk_ifgs_volcanoes.sh [-P] [-T] [-s foo.sh] $LiCSAR_procdir/subsets/Levee_Ramsey/165A [ifgs.list]"
 ...
 echo "parameter -T will add Tien Shan connection strategy"
 echo "----"
...
echo "framebatch_gapfill.sh -l -n 1 "$extra" -o 5 480" $rglks $azlks
framebatch_gapfill.sh -l -n 1 $extra -o 5 480 $rglks $azlks
